            ...
            "blocks": [
                {
                    ...
                    "label": {"type": "plain_text", "text": "책 제목"},
                    "element": {
                ...
                },
                {
                    ...
                    "label": {"type": "plain_text", "text": "오늘의 문장"},
                    "element": {
                        "type": "plain_text_input",
                        "action_id": "input_action_id",
                        "multiline": True,
                ...
                },
                {
                    ...
                    "label": {"type": "plain_text", "text": "생각 남기기"},
                    "optional": True,
                    "element": {
                        "type": "plain_text_input",
                        "action_id": "input_action_id",
...
def handle_view_submission_events(ack, body, client: WebClient):
    ...
    if channel_id != os.environ["BOOK_CHANEL"]:
        ack(
    ...
    text = (
        f">>> *<@{user_id}>님이 `{book_title}`에서 뽑은 오늘의 문장*\n\n '{sentence}'\n"
    )
...
def handle_admin_command(ack, body, client: WebClient):
    ...
    # 관리자인지 확인 후 아니라면 메세지 전송 후 종료
    user_id = body["user_id"]
    if user_id != os.environ["MANAGE_CHANEL"]:
...
def handel_some_action(ack, body, client: WebClient):
    ack()
    # 관리자의 DM 채널 ID 가져오기
